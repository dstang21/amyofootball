...
class SleeperSync
{
    ...
    public function __construct($leagueId, $pdo)
    {
        ...
        if (!$pdo || !($pdo instanceof PDO)) {
            throw new Exception("Invalid PDO connection provided to SleeperSync");
        }
    ...
    public function syncBasics()
    {
        try {
            $this->log("Starting basic sync for league: " . $this->leagueId);
            ...
            $this->log("Basic sync completed successfully for league: " . $this->leagueId);
            return ['success' => true, 'message' => 'Basic sync completed for league ' . $this->leagueId];
        } catch (Exception $e) {
            $this->log("Basic sync error: " . $e->getMessage());
            return ['success' => false, 'message' => $e->getMessage()];
        }
    }
    ...
    private function syncRosters()
    {
        ...
        $this->log("Syncing " . count($rosters) . " rosters for league {$this->leagueId}");
        ...
        foreach ($rosters as $roster) {
            try {
                ...
                $success = $stmt->execute([
                    $this->leagueId,
                    $roster['roster_id'],
                    $roster['owner_id'] ?? 'PLACEHOLDER_USER', // Handle null owner_id for players who left
                ...
                $this->log("Roster {$roster['roster_id']} - Rows affected: $rowsAffected");
                
            } catch (PDOException $e) {
                $this->log("Failed to sync roster {$roster['roster_id']}: " . $e->getMessage());
            }
        }
        
        $this->log("Finished syncing rosters for league {$this->leagueId}");
    }
    ...
    private function syncTransactions()
    {
        try {
            ...
            // Handle case where no transactions exist (empty array)
            if (empty($transactions)) {
                $this->log("No transactions found for league {$this->leagueId}");
                return;
            }
            ...
            $this->log("Failed to sync transactions: " . $e->getMessage());
    ...
    private function apiCall($endpoint)
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception("Invalid JSON response from: $url - Response: " . substr($response, 0, 200));
        }
